...
class MenuGenerator {
    constructor() {
        this.bedrockClient = new bedrock_client_1.BedrockClient();
    ...
    async generateWithBedrock(profiles, inventory, expiringItems, input) {
        ...
        if (response.dinners && Array.isArray(response.dinners)) {
            response.dinners.forEach((aiMeal, index) => {
        ...
        if (response.schoolLunches && Array.isArray(response.schoolLunches)) {
            response.schoolLunches.forEach((lunch) => {
                meals.push({
    ...
    generateMockMeals(weekStartDate) {
        const weekStart = weekStartDate || this.getNextMonday();
        const meals = [];
        // Generate 7 dinners
        const dinnerRecipes = [
            { name: 'Spaghetti Carbonara', time: 25, difficulty: 'beginner' },
            { name: 'Chicken Stir-Fry', time: 30, difficulty: 'beginner' },
            { name: 'Baked Salmon with Vegetables', time: 35, difficulty: 'intermediate' },
            { name: 'Beef Tacos', time: 20, difficulty: 'beginner' },
            { name: 'Vegetable Curry', time: 40, difficulty: 'intermediate' },
            { name: 'Homemade Pizza', time: 45, difficulty: 'intermediate' },
            { name: 'Roast Chicken with Potatoes', time: 60, difficulty: 'advanced' }
        ];
    ...
    buildInventoryAndShoppingList(meals, currentInventory, expiringItems) {
        ...
        ingredientMap.forEach((needed, name) => {
            ...
            if (inventoryItem) {
                ...
                if (usedFromInventory > 0) {
                    const isExpiring = expiringItems.some(exp => exp.name.toLowerCase() === name);
                    inventoryUsage.push({
